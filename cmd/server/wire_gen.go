...
func wireApp(confServer *conf.Server, confData *conf.Data, logger log.Logger) (*kratos.App, func(), error) {
	...
	runningFilterNameResolver := data.NewRunningFilterNameResolver(dataData)
	foDashboardUseCase := biz.NewFoDashboardUseCase(foDashboardRepo, runningFilterNameResolver)
	...
	aiDashboardUseCase := biz.NewAiDashboardUseCase(foDashboardUseCase, doDashboardUseCase, llmRepo)
	...
	alertUseCase := biz.NewAlertUseCase(confData)
	alertService := service.NewAlertService(alertUseCase)
	issueReportRepo := data.NewIssueReportRepo(dataData)
	issueReportUseCase := biz.NewIssueReportUseCase(issueReportRepo, logger)
	issueReportService := service.NewIssueReportService(issueReportUseCase)
	reconcileRepo := data.NewReconcileRepo(dataData)
	reconcileUseCase := biz.NewReconcileUseCase(reconcileRepo)
	reconcileService := service.NewReconcileService(reconcileUseCase)
	v := route.RegisterHttpService(confData, greeterApiService, foDashboardService, doDashboardService, aiDashboardService, etlService, alertService, issueReportService, reconcileService)
