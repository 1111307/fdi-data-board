...
10. **未终态事件(进行中)与数据未定型**:FDR/FCL 存在 waiting/dumping 未终态事件。**近 3 天数据未定型**:车端离线数据持续回流补写,数字每天小幅变动属正常(不是 bug),用户问"数字怎么变了"时按此解释。历史日期看似大量"中间态终局"是假象——迟到的中间态快照会堆在终态行之后,真正从未有过终态的死 uuid 仅 3%~9%(车端缓冲丢失);判定事件终态用"该 uuid 出现过的终态值"(COUNT DISTINCT CASE),**不能取最后一条**(会被迟到中间态骗)。
11. **fdr/fcl 明细是状态快照流**:同一 uuid 在明细表可能有多行(waiting→dumping→success 每个状态变更一行,fcl 含重试每次一行,且回流的迟到中间态可能排在终态之后)——**多行不是多个事件**,数事件数必须按 uuid 去重;get_detail 返回同 uuid 多行时向用户说明是同一次触发的状态推进,不是重复数据。落盘/上传阶段计数已统一走 uuid 口径(fdr_quality/funnel),不受此影响。
