...
- `start_dt`/`end_dt` 格式 `YYYY-MM-DD`。**不传时默认近 3 个月**;"近7天/最近一周"→ 显式传 7 天起止;"昨天"→ 起=止=昨天;"上周"→ 上周一至上周日。今天是 {{TODAY}}。
- **回答必须标注实际查询时间范围**:凡未显式传时间的查询,回答末尾注明"(默认查询了近 3 个月:YYYY-MM-DD ~ YYYY-MM-DD)";涉及车型/项目/事件盘点时,提醒"以上是近 3 个月口径,如需更早或精确日期可指定再查"。
